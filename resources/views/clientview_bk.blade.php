...
<div style="height:100%; width:98%; margin:10px;">
      ...
      <div id="proposalBillboards">
        <table class="table">
          ...
          @if(count($active_proposal_billboards))
            ...
            ?>
            <tr style="background-color:@if($active_proposal_billboard->client_accepted == 1) {{ '#c2dcbf;' }} @endif @if($active_proposal_billboard->client_rejected == 1) {{ '#f8bfb8;' }} @endif ">
              <td><?php echo $counter++; ?><input type="text" name="proposal_billboards[]" style="display:none;" value="{{ $active_proposal_billboard->id }}"></td>
              <td><img width="30" src="{{URL::to('/')}}/images/billboard.jpg"></td>
              <td><?php echo $active_proposal_billboard->name; ?></td>
              <td>$<?php echo number_format($active_proposal_billboard->monthly_price,2); ?></td>
              <td>
                @if($active_proposal_billboard->client_accepted == 0)
                <a href="{{ URL::to('/') }}/clientview/accept/<?php echo $active_proposal_billboard->proposal_id; ?>/<?php echo $active_proposal_billboard->apbid; ?>" data-confirm="Are you sure you want to ACCEPT this billboard for your proposal?">Accept</a>
                @endif
              </td>
              <td>
                @if($active_proposal_billboard->client_rejected == 0)
                <a href="{{ URL::to('/') }}/clientview/reject/<?php echo $active_proposal_billboard->proposal_id; ?>/<?php echo $active_proposal_billboard->apbid; ?>" data-confirm="Are you sure you want to REJECT this billboard for your proposal?">Reject</a></td>
                @endif
            </tr>
            @endforeach
          @endif
